...
pub fn build(b: *std.Build) void {
    ...
    if (enable_lua) {
        ...
        // Add Lua weak references demo
        const lua_weak_refs_example = b.addExecutable(.{
            .name = "lua_weak_references_demo",
            .root_source_file = b.path("examples/lua_weak_references_demo.zig"),
            .target = target,
            .optimize = optimize,
        });

        lua_weak_refs_example.root_module.addImport("zig_llms", lib.root_module);
        lua_weak_refs_example.linkLibrary(lib);
