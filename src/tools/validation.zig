    ...
    fn validateMetadata(
        ...
        if (!isValidToolName(metadata.name)) {
            try errors.append(.{
                ...
                .message = try std.fmt.allocPrint(self.allocator, 
                    "Invalid tool name: '{s}'. Must contain only alphanumeric, underscore, and dash",
                    .{metadata.name}),
            });
        }
        ...
        if (!isValidVersion(metadata.version)) {
            try errors.append(.{
                ...
                .message = try std.fmt.allocPrint(self.allocator,
                    "Invalid version format: '{s}'. Expected semver format (e.g., 1.0.0)",
                    .{metadata.version}),
            });
        }
    ...
    fn testExecution(
        ...
        const test_input = createTestInput(self.allocator, tool.metadata.input_schema) catch |err| {
            try warnings.append(.{
                .kind = .other,
                .message = try std.fmt.allocPrint(self.allocator,
                    "Could not generate test input: {s}",
                    .{@errorName(err)}),
            });
            return;
        };
        ...
        const output_validation = tool.metadata.output_schema.validate(result) catch |err| {
            try errors.append(.{
                .kind = .invalid_schema,
                .message = try std.fmt.allocPrint(self.allocator,
                    "Output does not match schema: {s}",
                    .{@errorName(err)}),
        ...
        if (self.config.max_execution_time_ms) |max_time| {
            if (execution_time > max_time) {
                try warnings.append(.{
                    ...
                    .message = try std.fmt.allocPrint(self.allocator,
                        "Execution time ({d}ms) exceeds recommended maximum ({d}ms)",
