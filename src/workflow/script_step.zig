...
pub const ScriptStepExecutor = struct {
    ...
    fn executeShell(
        ...
        child.spawn() catch |err| {
            ...
            result.error_message = try std.fmt.allocPrint(self.allocator, "Failed to spawn shell process: {s}", .{@errorName(err)});
            return;
        };
        ...
        const term = child.wait() catch |err| {
            result.success = false;
            result.error_message = try std.fmt.allocPrint(self.allocator, "Failed to wait for shell process: {s}", .{@errorName(err)});
            return;
        };
        ...
        switch (term) {
            ...
            .Signal => |signal| {
                result.success = false;
                result.error_message = try std.fmt.allocPrint(self.allocator, "Process terminated by signal: {d}", .{signal});
            },
            .Stopped => |signal| {
                result.success = false;
                result.error_message = try std.fmt.allocPrint(self.allocator, "Process stopped by signal: {d}", .{signal});
            },
            .Unknown => |code| {
                result.success = false;
                result.error_message = try std.fmt.allocPrint(self.allocator, "Process terminated with unknown code: {d}", .{code});
            },
        }
    }
    ...
    fn executeExternalScript(
        ...
        child.spawn() catch |err| {
            result.success = false;
            result.error_message = try std.fmt.allocPrint(self.allocator, "Failed to spawn {s}: {s}", .{ interpreter, @errorName(err) });
            return;
        };
        ...
        const term = child.wait() catch |err| {
            result.success = false;
            result.error_message = try std.fmt.allocPrint(self.allocator, "Failed to wait for {s}: {s}", .{ interpreter, @errorName(err) });
            return;
        };
        ...
        switch (term) {
            ...
            .Signal => |signal| {
                result.success = false;
                result.error_message = try std.fmt.allocPrint(self.allocator, "{s} terminated by signal: {d}", .{ interpreter, signal });
            },
            .Stopped => |signal| {
                result.success = false;
                result.error_message = try std.fmt.allocPrint(self.allocator, "{s} stopped by signal: {d}", .{ interpreter, signal });
            },
            .Unknown => |code| {
                result.success = false;
                result.error_message = try std.fmt.allocPrint(self.allocator, "{s} terminated with unknown code: {d}", .{ interpreter, code });
            },
        }
    }
