...
pub const WorkflowStateManager = struct {
    ...
    pub fn createCheckpoint(self: *WorkflowStateManager, context: *WorkflowExecutionContext, checkpoint_id: []const u8) !void {
        ...
        const checkpoint_key = try std.fmt.allocPrint(self.allocator, "{s}:{s}:checkpoint:{s}", .{ self.workflow_id, self.instance_id, checkpoint_id });
    ...
    pub fn listCheckpoints(self: *WorkflowStateManager) ![][]const u8 {
        const pattern = try std.fmt.allocPrint(self.allocator, "{s}:{s}:checkpoint:*", .{ self.workflow_id, self.instance_id });
